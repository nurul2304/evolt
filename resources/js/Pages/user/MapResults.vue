...
const goToStatusCharging = () => {
    router.visit('/status-charging', {
        data: {
            stationName: selectedStation.value.name,
            location: selectedStation.value.location,
            duration: estimatedDurationMinutes.value,
            power: selectedStation.value.power,
            type: selectedStation.value.chargers[0], // Assuming first charger type
        }
    });
};
    ...
    <div class="min-h-screen flex flex-col bg-gray-50 font-sans text-gray-800">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div class="flex flex-col md:flex-row justify-end items-start md:items-center mb-6">
                    <div class="relative w-full md:w-72 group">
                        <input type="text" placeholder="Cari Stasiun Lain..."
                            ...
                            @click="openModal" readonly value="Cari Stasiun Lainnya...">
                        <i
                            class="fas fa-search absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 group-hover:text-[#00C853] transition-colors"></i>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="absolute inset-0">
                        <div class="w-full h-full relative">
                            <div
                                class="absolute left-1/2 top-4 transform -translate-x-1/2 text-center bg-white/90 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-sm z-[500]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                    <div v-for="station in nearestStations" :key="station.id"
                        ...
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <h2 class="text-lg sm:text-xl font-bold text-gray-900 leading-tight mb-1">{{
                                    station.name }}</h2>
                        ...
                        <div class="mt-auto space-y-4">
                            ...
                            <button @click="reserveStation(station.id)" :disabled="!station.isBookable" :class="[
        ...
        <Transition name="slide-up">
            <div v-if="showSearchModal"
                ...
                <div class="bg-white w-full h-[90vh] sm:h-auto sm:max-w-2xl rounded-t-[2rem] sm:rounded-3xl p-6 sm:p-8 shadow-2xl transform transition-transform duration-300 sm:duration-200 flex flex-col relative touch-none sm:touch-auto"
                    :style="{ transform: isDragging ? `translateY(${dragOffset}px)` : '' }">
                    <div class="w-full h-8 absolute top-0 left-0 z-50 flex justify-center items-center sm:hidden cursor-grab active:cursor-grabbing"
                        @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd">
                        <div class="w-14 h-1.5 bg-gray-300 rounded-full"></div>
                    </div>

                    <div class="mt-4 sm:mt-0 h-full flex flex-col">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">Cari Stasiun</h3>

                        <form @submit.prevent="submitSearch"
                            class="space-y-5 overflow-y-auto flex-grow custom-scrollbar px-1">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Merk</label>
                                    <div @click.stop="openOnly('brand')" class="dropdown-trigger"
                                        :class="{ 'active': isBrandOpen }">
                                        <span class="text-gray-800 truncate">{{ formState.brand || 'Pilih Merk'
                                        }}</span>
                                    ...
                                    <div v-if="isBrandOpen" class="dropdown-menu">
                                        <div v-for="opt in brandOptions" :key="opt" @click="selectOption('brand', opt)"
                                            class="dropdown-item" :class="{ 'selected': formState.brand === opt }">{{
                                                opt
                                            }}</div>
                                    </div>
                                </div>
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Tipe</label>
                                    <div @click.stop="openOnly('type')" class="dropdown-trigger"
                                    ...
                                    <div v-if="isTypeOpen" class="dropdown-menu">
                                        <div v-for="opt in typeOptions" :key="opt" @click="selectOption('type', opt)"
                                            class="dropdown-item" :class="{ 'selected': formState.type === opt }">{{ opt
                                            }}</div>
                                    </div>
                                </div>
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Domisili</label>
                                    <div @click.stop="openOnly('domicile')" class="dropdown-trigger"
                                        :class="{ 'active': isDomicileOpen }">
                                        <span class="text-gray-800 truncate">{{ formState.domicile || 'Pilih Area'
                                        }}</span>
                                        <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                                            :class="{ 'rotate-180': isDomicileOpen }"></i>
                                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Stasiun</label>
                                    <div @click.stop="openOnly('station')" class="dropdown-trigger"
                                        :class="{ 'active': isStationOpen }">
                                        <span class="text-gray-800 truncate">{{ formState.station || 'Semua Stasiun'
                                        }}</span>
                                        <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                                            :class="{ 'rotate-180': isStationOpen }"></i>
                                    </div>
                                    <div v-if="isStationOpen" class="dropdown-menu">
                                        <div v-for="opt in stationOptions" :key="opt"
                                            @click="selectOption('station', opt)" class="dropdown-item"
                                            :class="{ 'selected': formState.station === opt }">{{ opt }}</div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            class="pt-4 mt-auto border-t border-gray-100 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
                            <button type="button" @click="closeModal"
                                class="w-full sm:w-auto px-6 py-3.5 rounded-xl text-gray-600 font-semibold hover:bg-gray-100 transition">
                                Batal
                            </button>
                            <button type="button" @click="submitSearch"
                                class="w-full sm:w-auto bg-[#00C853] text-white font-bold px-8 py-3.5 rounded-xl hover:bg-[#00A142] active:scale-95 transition shadow-lg shadow-[#00C853]/20">
                                Terapkan Filter
                            </button>
                        </div>
                    </div>
        ...
        <Transition name="slide-up">
            <div v-if="showConfirmationModal && selectedStation"
                ...
                <div class="bg-white w-full max-h-[85vh] flex flex-col rounded-t-[2rem] sm:rounded-3xl shadow-2xl md:max-w-4xl transform transition-transform duration-300 sm:duration-200 relative touch-none sm:touch-auto"
                    :style="{ transform: isDragging ? `translateY(${dragOffset}px)` : '' }">
                    ...
                    <div class="flex-1 overflow-y-auto px-6 sm:px-8 custom-scrollbar pb-4">
                        <div class="space-y-4 mt-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                ...
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Waktu
                                        Mulai</label>
                                    <div id="time-trigger" @click.stop="openOnly('time')" class="dropdown-trigger"
                                        ...
                                        <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                                            :class="{ 'rotate-180': isTimeOpen }"></i>
                                    </div>
                                    <div v-if="isTimeOpen" class="dropdown-menu">
                                        <div v-for="time in timeOptions" :key="time.value"
                                            @click="selectOption('time', time.value)" class="dropdown-item"
                                            :class="{ 'selected': selectedStartTime === time.value }">{{ time.label }}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Port
                                        Charging</label>
                                    <div id="port-trigger" @click.stop="openOnly('port')" class="dropdown-trigger"
                                        :class="{ 'active': isPortOpen }">
                                        <span class="text-gray-800 truncate">{{selectedPort ? portOptions.find(p =>
                                            p.value === selectedPort)?.label : 'Pilih Port'}}</span>
                                        <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                                            :class="{ 'rotate-180': isPortOpen }"></i>
                                    </div>
                                ...
                                <div class="relative">
                                    <label
                                        class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Target
                                        Energi (kWh)</label>
                                    <div id="duration-trigger" @click.stop="openOnly('duration')"
                                        class="dropdown-trigger" :class="{ 'active': isDurationOpen }">
                                        <span class="text-gray-800 truncate">{{durationOptions.find(d => d.value ===
                                            selectedDuration)?.label || 'Pilih Energi'}}</span>
                                        <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform"
                                            :class="{ 'rotate-180': isDurationOpen }"></i>
                                    </div>
                                    <div v-if="isDurationOpen" class="dropdown-menu">
                                        <div v-for="duration in durationOptions" :key="duration.value"
                                            @click="selectOption('duration', duration.value)" class="dropdown-item"
                            ...
                            <p class="text-xs text-gray-400 mt-1">
                                *Waktu mulai fleksibel (Hari Ini); sistem akan mengecek ketersediaan port setelah
                                booking lain selesai.
                            </p>

                            <div v-if="estimatedDurationMinutes > 0" class="flex flex-col justify-end md:mt-0 mt-2">
                                <div
                                    class="p-3 bg-green-50 rounded-xl flex justify-between text-sm border border-green-200 w-full">
                                    <div class="flex flex-col">
                                        <span class="text-gray-500 flex items-center gap-1">
                                            <i class="fas fa-hourglass-half text-green-600"></i> Est. Durasi
                                        </span>
                                        <span class="font-bold text-green-700 text-lg">{{ estimatedDurationMinutes }}
                                            menit</span>
                                    </div>
                            ...
                            <div v-else
                                class="text-sm text-gray-400 p-3 flex items-center justify-center border border-dashed border-gray-300 rounded-xl md:mt-0 mt-2">
                            ...
                            <div class="bg-gray-50 rounded-2xl p-4 space-y-3 border border-gray-100">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-semibold text-gray-800 text-right">{{ selectedStation.name
                                    }}</span>
                                </div>
                                <div class="flex justify-between items-center text-sm">
                                ...
                                <div class="border-t border-gray-200 pt-3 space-y-2">
                                    <div class="flex justify-between items-center text-sm">
                                        ...
                                        <span class="font-medium text-gray-800">{{
                                            formatRupiah(priceBreakdown.pricePerKwh) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-sm">
                                        <span class="text-gray-500">Biaya Energi</span>
                                        <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.price)
                                        }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-sm">
                                        <span class="text-gray-500">Biaya Layanan</span>
                                        <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.service)
                                        }}</span>
                                    </div>
                                    <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                                        <span class="font-bold text-gray-900">Total</span>
                                        <span class="font-bold text-xl text-[#00C853]">{{ calculateTotalFormatted
                                        }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div
                        class="flex-none px-6 pb-6 sm:px-8 sm:pb-8 pt-4 bg-white rounded-b-[2rem] sm:rounded-b-3xl z-10">
                        <div class="flex flex-col-reverse sm:flex-row gap-3">
                            <button type="button" @click="cancelProcess"
                                class="w-full py-3.5 rounded-xl font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 transition">
                                Batal
                            </button>
                            <button type="button" @click="proceedToPayment"
                                :disabled="!selectedPort || !isStartTimeValid || estimatedDurationMinutes === 0"
                ...
                <div class="bg-white rounded-3xl p-6 shadow-2xl w-full max-w-md relative overflow-hidden">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button @click="cancelProcess"
                            class="bg-gray-100 rounded-full p-2 text-gray-500 hover:bg-gray-200 transition">
                    ...
                    <div class="flex flex-col items-center mb-6">
                        <div
                            class="bg-white p-4 rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.1)] border border-gray-100 mb-4">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=QRIS%20Payment"
                    ...
                    <div class="p-8 text-center">
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 border-2 border-dashed border-gray-200 mb-6">
                            ...
                            <div class="text-2xl font-mono font-bold text-gray-800 tracking-wider">{{
                                selectedStation.bookingNumber }}</div>
                        </div>

                        <div class="bg-white p-2 rounded-xl border border-gray-100 inline-block mb-4 shadow-sm">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=Access"
                                alt="QR Access" class="w-32 h-32">
                        </div>
                        <p class="text-xs text-gray-400">Scan QR ini pada mesin charging</p>
                    </div>

                    <div class="p-4 bg-gray-50 border-t border-gray-100">
                        <button @click="goToStatusCharging"
                            class="w-full py-3.5 bg-[#00C853] text-white font-bold rounded-xl hover:bg-[#00A142] shadow-lg active:scale-95 transition mb-3">
                            <i class="fas fa-bolt mr-2"></i> Lihat Status Charging
                        </button>
                        <button @click="openPrintStruk"
                            class="w-full py-3.5 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 shadow-lg active:scale-95 transition mb-3">
                            <i class="fas fa-download mr-2"></i> Simpan Struk
                        </button>
                        <button @click="closeReceiptModal"
                            class="w-full py-3.5 text-gray-500 font-bold hover:text-gray-800 transition">
                            Tutup
                        </button>
                    </div>
                </div>
